...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelector('#resetRowProgress').addEventListener('click',
        async (event) => {
            ...
            [durationPG, cutEndTimes].forEach(t => t.textContent = formatTime(d));
    ...
    function resetPopup() {
        ...
        .forEach(el => el.textContent = 'unknown');
        ...
        a.textContent = 'No songs are playing';
        a.classList.remove('active');

        if (p) {
            p.style.transition = "background 1s ease";
            p.style.background = "rgba(52,59,60)";
            p.querySelectorAll("*").forEach(txt => (txt.style.color = "#fff"));
        }
    }
    ...
    (function () {
        ...
        function setAppend(state = false) {
            const text = [inputIds.preTitle,
                inputIds.preArtist];
            const set = (color = '', btn = '') => {
                text.forEach(t => t.style.color = color);
