...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const anTE = document.querySelector("#active-track-animations");
    ...
    function showToast(message, duration = 900) { if (!toastEnabled) return;
    ...
    function updateRangeFade(progress) {
        if (!progress) {
            [fadeInSlider,
                fadeOutSlider].forEach(e => {
                    e.value = 0;
                    e.style.background = '';
                });
                
            [fadeInValue,
                fadeOutValue].forEach(e => e.textContent = '0.0s');
    ...
    async function resetActivityEdit() {
        try {
            ...
            audioProgres.style.background = '';
            [durationPG, cutEndTimes].forEach(e => e.textContent = formatTime(0));
            duresult.forEach(e =>
            ...
            const loadAudio = new Promise((resolve, reject) => {
                ...
                cutAudioPlayer.onloadedmetadata = async () => {
                    ...
                    startSlider.value = audioProgres.value = 0;
                     [durationPG, cutEndTimes].forEach(e => e.textContent = formatTime(d));
    ...
    function resetPopup() {
        ...
        if (anTE) {
            anTE.textContent = 'No songs are playing';
            anTE.classList.remove('active');
        }
        if (playerItem) {
            playerItem.style.transition = "background 1s ease";
    ...
    function popupTrackInfo(track, img) {
        ...
        if (anTE) {
            anTE.textContent = `${track.title} - ${track.artist}`;
            anTE.classList.add('active');
        }
    ...
    (function() {
        ...
        document.querySelector('#now-playing-preview').addEventListener('click',
            (e) => { e.stopPropagation(); viewPopupMenu.classList.add('active');
            });
        ...
        filterBtn.addEventListener('click', (e) => {e.stopPropagation(); navigation(); hideFilter();
        });
        document.querySelector('#menuBtn').addEventListener("click", () => navigation(sideMenu));
        document.addEventListener("click", (e) => { if (e.target === sideMenu) {
    ...
    document.querySelector('#installNowBtn')?.addEventListener('click',
        async () => {
            if (!isPrompt) return;
